...
화면 위쪽 띠에 뜨는 점 대신 그 사람의 스킨 얼굴을 띄운다.

| 파일 | 하는 일 |
|---|---|
| `face-pack.ps1` | 닉네임으로 Mojang에서 스킨을 받아 얼굴을 오려 내고, 거리별 네 단계(8·7·6·5픽셀)를 만들어 `DB.zip`·`DB_full.zip`·서버의 풀어 둔 팩에 넣는다 |

모양 이름은 `taggame:face_<소문자 닉>`이며, 누구에게 붙일지는 서버 `plugins/TagGame/config.yml`의
`locator-bar.faces` 목록이 정한다. 거리에 따라 크기를 고르는 것은 마인크래프트가 알아서 한다
(목록의 앞쪽이 가까울 때).

**스킨은 저절로 따라 바뀌지 않는다.** 누가 스킨을 바꿨으면 그 닉네임만 넘겨 다시 돌리고
아래 갱신 절차를 따른다.
...
얼굴을 쓰는 사람은 `locator-bar.colors`의 색이 무시된다 — 그림 그대로의 색이 나와야 하므로
플러그인이 점 색을 흰색으로 고정한다.

## 블랙잭 카드

| 파일 | 하는 일 |
|---|---|
| `blackjack-cut.ps1` | 바탕화면 `블랙잭 카드` 폴더의 원본 두 장(`포커.png`·`덮힌카드.png`)을 잘라 카드 53장을 `blackjack` 폴더에 만든다 |
| `blackjack` 폴더 | 잘라 낸 카드 그림 (64x64 정사각, 여백은 투명). `.gitignore`로 저장소에는 들어가지 않는다 |
| `blackjack-pack.ps1` | 위 그림을 `taggame:card_*` 자산으로 만들어 `DB.zip`·`DB_full.zip`·서버의 풀어 둔 팩 세 곳에 넣는다 |

꼬리잡기 플러그인이 종이 아이템에 `taggame:card_<무늬>_<끗>`을 붙여 카드를 보여 준다.
무늬는 `h`(하트)·`d`(다이아)·`c`(클로버)·`s`(스페이드), 끗은 `a`·`2`~`10`·`j`·`q`·`k`, 뒷면은 `card_back`이다.

원본 그림을 고쳤으면 `blackjack-cut.ps1` → `blackjack-pack.ps1` 순서로 돌리고 아래 갱신 절차를 따른다.

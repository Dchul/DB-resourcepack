...
| 파일 | 하는 일 |
|---|---|
| `gacha-pack.ps1` | `RR.zip` 안 뽑기 기계 3종의 몸통을 꺼내 `DB.zip`·`DB_full.zip`에 넣는다 |

`/뽑기기계 1|2|3`으로 세우는 기계의 모양이다. 원본은 `RR.zip`에 ModelEngine이 구워 둔 것이지만,
그 자리에 두면 `me-pack.ps1`이 ModelEngine 쪽을 통째로 갈아엎을 때 함께 지워진다.
그래서 `assets/taggame/…/gacha_1~3`으로 옮겨 담아 쓴다.

움직임 원본(`.bbmodel`)이 없어 **몸통 하나만** 쓴다. 따로 노는 작은 조각(튀어나오는 캡슐, 누르는 손잡이)은
어디에 붙는지 알 수 없어 빠져 있다. 크기와 세우는 자리는 플러그인 쪽 `GachaMachines`의 표에서 정한다.
